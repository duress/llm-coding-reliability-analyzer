...
def calculate_ai_human_disagreement_rate(df: pd.DataFrame, ai_columns: List, human_column: str) -> Tuple[float, int, int, List[Tuple[int, str, List[str], str]]]:
    """
    Calculate LLM-Human disagreement rate: proportion of cases where any LLM coding run disagrees with human standard coding
    """
    ...
    for i in range(len(ai_codes)):
        ...
        # Check if any LLM coding run disagrees with human
...
def calculate_inter_rater_reliability(df: pd.DataFrame, ai_columns: List, human_column: str) -> Tuple[float, List[float], float, List[float], List[Tuple[int, str, List[str]]], dict, Tuple[float, int, int, List]]:
    """Calculate inter-rater reliability (LLM vs. Human) and intra-rater reliability (LLM repeated coding consistency)"""
    ai_codes = df[ai_columns].values
    human_codes = df[human_column].values
    comments = df.get('comment', pd.Series(["No comment available"] * len(df))).values
    ...
    # Intra-rater reliability: LLM coding consistency across repeated runs
    intra_kappas = []
    ...
    # LLM internal inconsistency cases (when LLM coding runs disagree among themselves)
    ai_internal_disagreements = []
    for i in range(len(ai_codes)):
        if len(set(ai_codes[i])) > 1:  # LLM coding runs disagree among themselves
...
def analyze_coding_agreement(file_path: str, delimiter='\t', encoding='utf-8', ai_columns=None, human_column=None, valid_categories=['a', 'b']):
    """
    Analyze LLM coding reliability through repeated coding runs, prioritizing pairwise Cohen's Kappa,
    using human coding as final result when LLM coding runs show inconsistency.
    
    Parameters:
    file_path: Path to the file
    delimiter: Delimiter (default: tab)
    encoding: File encoding (default: utf-8)
    ai_columns: LLM coding run column indices or names (default: first five columns for five repeated runs)
    ...
    # Unpack LLM-Human disagreement info
    ...
    ai_internal_unanimous = len(df) - len(ai_internal_disagreements)  # Cases where all LLM coding runs agree
    ai_human_agreement = total_cases - ai_human_disagreement_count  # Cases where all LLM runs agree with human
    
    # Output analysis results
    print("\n=== Analysis Summary ===")
    print(f"Number of LLM coding runs: {len(ai_columns)}")
    print(f"Total cases: {total_cases}")
    
    print(f"\n=== Kappa Statistics ===")
    print(f"Average inter-rater Kappa (LLM vs. Human): {inter_mean_kappa:.4f}")
    for i, kappa in enumerate(inter_kappas, 1):
        print(f"  LLM Run {i} vs Human: {kappa:.4f}")
    
    print(f"\nAverage intra-rater Kappa (LLM coding consistency): {intra_mean_kappa:.4f}")
    idx = 0
    for i in range(len(ai_columns)):
        for j in range(i + 1, len(ai_columns)):
            print(f"  LLM Run {i+1} vs Run {j+1}: {intra_kappas[idx]:.4f}")
            idx += 1
    ...
    print(f"LLM-Human Disagreement Rate: {ai_human_disagreement_rate:.4f} ({ai_human_disagreement_count}/{total_cases} cases)")
    print(f"  Definition: Proportion of cases where any LLM coding run disagrees with human standard coding")
    print(f"LLM Internal Inconsistency: {len(ai_internal_disagreements)}/{total_cases} cases where LLM coding runs disagree among themselves")
    print(f"LLM Internal Consistency: {ai_internal_unanimous}/{total_cases} cases where all LLM coding runs agree")
    print(f"Perfect Agreement (All LLM runs + Human): {ai_human_agreement}/{total_cases} cases")
    
    if ai_human_disagreements:
        print(f"\nLLM-Human disagreement cases (showing first 5 of {len(ai_human_disagreements)}):")
        for idx, comment, ai_codes, human_code in ai_human_disagreements[:5]:
            print(f"  Row {idx}: LLM coding runs: {ai_codes}, Human code: {human_code}, Comment: {comment}")
    else:
        print("\nNo LLM-Human disagreement cases found")
    
    if ai_internal_disagreements:
        print(f"\nLLM internal inconsistency cases (showing first 5 of {len(ai_internal_disagreements)}):")
        for idx, comment, ai_codes in ai_internal_disagreements[:5]:
            print(f"  Row {idx}: LLM coding runs: {ai_codes}, Comment: {comment}")
    else:
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"LLM coding runs: {len(ai_columns)}, Human coder: 1\n\n")
        
        f.write(f"=== Kappa Statistics ===\n")
        f.write(f"Average inter-rater Kappa (LLM vs. Human): {inter_mean_kappa:.4f}\n")
        for i, k in enumerate(inter_kappas, 1):
            f.write(f"LLM Run {i}-Human Kappa: {k:.4f}\n")
        f.write(f"\nAverage intra-rater Kappa (LLM coding consistency): {intra_mean_kappa:.4f}\n")
        idx = 0
        for i in range(len(ai_columns)):
            for j in range(i + 1, len(ai_columns)):
                f.write(f"LLM Run {i+1}-Run {j+1} Kappa: {intra_kappas[idx]:.4f}\n")
                idx += 1
        f.write(f"\nAverage Kappa for each coder (including comparison with human):\n")
        for col, mean_kappa in coder_mean_kappa.items():
        ...
        f.write(f"\n=== Disagreement Analysis ===\n")
        f.write(f"LLM-Human Disagreement Rate: {ai_human_disagreement_rate:.4f} ({ai_human_disagreement_count}/{total_cases} cases)\n")
        f.write(f"Definition: Proportion of cases where any LLM coding run disagrees with human standard coding\n")
        f.write(f"LLM Internal Inconsistency: {len(ai_internal_disagreements)}/{total_cases} cases\n")
        f.write(f"LLM Internal Consistency: {ai_internal_unanimous}/{total_cases} cases\n")
        f.write(f"Perfect Agreement (All LLM runs + Human): {ai_human_agreement}/{total_cases} cases\n\n")
        
        if ai_human_disagreements:
            f.write("=== LLM-Human Disagreement Cases ===\n")
            for idx, comment, ai_codes, human_code in ai_human_disagreements:
                f.write(f"Row {idx}:\n")
                f.write(f"  LLM coding runs: {ai_codes}\n")
                f.write(f"  Human code: {human_code}\n")
                f.write(f"  Comment: {comment}\n")
                f.write(f"  Final code: {human_code}\n\n")
        else:
            f.write("No LLM-Human disagreement cases found\n")
        
        if ai_internal_disagreements:
            f.write("=== LLM Internal Inconsistency Cases ===\n")
            ...
            f.write("No LLM internal inconsistency cases found\n")
