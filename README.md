...
A user-friendly toolkit for **LLM-assisted Content Analysis (LACA)**, featuring two tools designed for prompt optimization and statistical analysis. Suitable for researchers and non-programmers working with small datasets.
...
- **Coding Quality Check Tool**: For quick testing and improving LLM prompts.
...
- Download Python 3.7 or higher from [python.org](https://www.python.org/).
...
1. Prepare your data as a tab-separated text file (see [Input Data Format](#input-data-format) above).
2. Place the file in the same folder as the Python scripts.
3. Open terminal in the toolkit folder.
4. Run the tool:

   ```bash
   python coding_quality_checker_EN.py
   ```
...
3. Run the tool:
   ...
   ```bash
   python statistical_reliability_comparator_EN.py
   ```
...
- Aim for Cohen's Kappa ≥ 0.8 for good reliability (following the standard adopted in Ko, 2026).
...
If you use this toolkit in your research, please cite the accompanying paper:

```
Ko, L.-Y. (2026). Bridging Statistics and Interpretation: An LLM-assisted 
Content Analysis Approach to Corpus-assisted Discourse Analysis. 
Mass Communication Research, 167, 123–180. 
https://doi.org/10.30386/MCR.202604.0008

Code repository: https://github.com/duress/llm-coding-reliability-analyzer
```
